...
import { createComment } from "../issue-actions";
    ...
    <div class="flex flex-col gap-4">
      <Index each={timelineItems()}>
        {(item) => {
          ...
          return (
            <Switch>
              <Match when={currentItem.type === "comment"}>
                <IssueCommentItem
                  comment={(currentItem as CommentTimelineItem).data}
                />
              </Match>
              <Match when={currentItem.type === "event"}>
                <IssueEventItem
                  event={(currentItem as EventTimelineItem).data}
                  assignedTo={(currentItem as EventTimelineItem).assignedTo}
                />
              </Match>
            </Switch>
          );
